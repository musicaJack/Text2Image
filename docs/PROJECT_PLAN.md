...
| 术语 | 定义 |
|------|------|
| **模型下拉** | 首页上的**下拉菜单**（`<select>`），供用户选择使用哪个文生图模型（如 Qwen-2.0-Pro、万相 2.6 等）。选项从 `GET /api/v1/models` 动态加载，用户选择后随生成请求一并提交给后端。 |

### 2.2 页面结构

| 页面 | 路径 | 说明 |
|------|------|------|
| 首页（生成） | `/` 或 `index.html` | 输入 prompt、**通过下拉菜单选择模型**、生成图片 |
...
- [ ] **1.8** 实现 `index.html`：输入框、**模型下拉菜单**（通过下拉选择模型）、生成按钮、图片展示
...
- [ ] **3.1** 首页模型下拉菜单的选项从 `/api/v1/models` 动态加载（而非写死在 HTML 中）
- [ ] **3.2** 加载状态、错误提示、超时处理
